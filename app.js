...
const editComment = (targetComment) => {
  ...
  targetComment
    .querySelector(".comment-tools")
    .insertAdjacentElement("beforebegin", btnUpdate);
...
const update = (targetComment, btn) => {
  ...
  if (commentGroupObj.id === targetId) {
    ...
    commentGroupObj.replies.find((obj) => obj.id === targetId).content =
      newContentText;
  }
...
const replyComment = (targetComment) => {
  ...
  if (targetComment.parentElement.classList.contains("replies-comments")) {
    ...
    targetComment.nextElementSibling
      ...
      .insertAdjacentElement("afterbegin", replyForm);
  }
  replyForm.focus();
...
const addReplyComment = (targetComment, form, content) => {
  ...
  const groupOgj = comments.get(
    targetComment.closest(".comment-replies-group").id
  );
...
const btnEditDeleteTemplate = document.getElementById(
  "btn-edit-delete-template"
);
...
const createCommentReplyGroup = (group) => {
  ...
  const mainComment = createComment(
    group,
